...
BumpGoNode::control_cycle()
{
  ...
  switch (state_) {
    ...
    case BACK:
      ...
      break;
    case TURN:
      // no se pone out_vel.linear.x = 0 porque se inicializa a 0 el mensaje "Twist" en cada 
      // iteración.
...
BumpGoNode::check_forward_2_back()
{
  ...
  size_t pos = 0;  // pos = 0: es tu centro del laser (size_t es un int pequeño)
  // (pos2 = last_scan_->ranges.size() --> (es el centro menos un incremento de angulo, laser 360°)
  // pequeño). La diagonal se divide entre 4 (se multiplica por 3 si quieres la de la dcha)
  // size_t pos2 = last_scan_->ranges.size() / 2; //PUNTO DE ATRAS 
  // std::cout << "pos1:" <<last_scan_->ranges[pos] << std::endl;

  // prueba HOY: size_t pos = last_scan_->ranges.size() + 45 --> donde lo detecte es el lado pos.
  std::cout << "pos:" <<last_scan_->ranges[pos] << std::endl;
